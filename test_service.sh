...
if [[ $? -eq 0 ]] && [[ "$RESPONSE" == *"status_list"* ]]; then
    ...
    if [[ -n "$STATUS_URI" ]] && [[ -n "$INDEX" ]]; then
        ...
        ENCODED_STATUS_URI=$(printf '%s' "$STATUS_URI" | sed -e 's|/|%2F|g' -e 's/:/%3A/g')

        # Test 2: Check initial status
        echo "🔍 Test 2: Checking initial status..."
        INITIAL_STATUS=$(curl -s "$BASE_URL/token_status_list/get?uri=$ENCODED_STATUS_URI&idx=$INDEX")
        ...
        if [[ "$INITIAL_STATUS" == "0" ]]; then
            ...
            JWT_RESPONSE=$(curl -s -D - -H "Accept: application/statuslist+jwt" "$STATUS_URI")
            ...
            CWT_RESPONSE=$(curl -s -D - -H "Accept: application/statuslist+cwt" "$STATUS_URI")
            ...
            if [[ "$REVOKE_RESPONSE" == *"Status Changed"* ]]; then
                ...
                FINAL_STATUS=$(curl -s "$BASE_URL/token_status_list/get?uri=$ENCODED_STATUS_URI&idx=$INDEX")
...
if [[ -n "$STATUS_URI" ]]; then
    ...
    JWT_RESPONSE=$(curl -s -H "Accept: application/statuslist+jwt" "$STATUS_URI")
    ...
    CWT_RESPONSE=$(curl -s -H "Accept: application/statuslist+cwt" "$STATUS_URI")
    ...
    DEFAULT_RESPONSE=$(curl -s "$STATUS_URI")
    ...
    INVALID_RESPONSE=$(curl -s -w "%{http_code}" -H "Accept: application/xml" "$STATUS_URI")
...
if [[ -n "$STATUS_URI" ]]; then
    ...
    JWT_HEADERS=$(curl -s -v -H "Accept: application/statuslist+jwt" "$STATUS_URI" 2>&1 | grep -i "content-type")
    ...
    CWT_HEADERS=$(curl -s -v -H "Accept: application/statuslist+cwt" "$STATUS_URI" 2>&1 | grep -i "content-type")
